...
def visualize_predictions(model, dataset, indices=[0, 1, 2], num_samples=3, device='cpu'):
    ...
    print("结束预测可视化")
    plt.tight_layout()
    plt.savefig('model/visual/predictions_epoch_{}.png'.format(epoch))
    plt.close()
...
if __name__ == "__main__":
    ...
    all_images = os.listdir('data/cutting/images')  # Make sure to use your actual directory path     
    ...
    train_dataset = SegmentationDataset('data/cutting/images', 'data/cutting/masks', train_files, transform=transform)
    test_dataset = SegmentationDataset('data/cutting/images', 'data/cutting/masks', test_files, transform=transform)
