...
def split_dataset(entire_image_dir, cutting_image_dir, test_ratio=0.2):
    """
    将完整图片和子图数据集分割为训练集和测试集
    
    :param entire_image_dir: 完整图片文件夹路径
    :param cutting_image_dir: 子图文件夹路径
    :param test_ratio: 测试集占总数据集的比例, 默认为0.1
    """
    
    # 获取完整图片文件名
    entire_images = os.listdir(entire_image_dir)  # 直接获取完整图片文件名
    
    # 获取子图文件名并构建字典
    sub_img_files = os.listdir(cutting_image_dir)
    sub_img_dict = {}
    
    for sub_img in sub_img_files:
        full_img_name = '_'.join(sub_img.split('_')[:-1])  # 获取完整图片名
        if full_img_name not in sub_img_dict:
            sub_img_dict[full_img_name] = []
        sub_img_dict[full_img_name].append(sub_img)

    # 随机选择测试集的完整图片
    entire_test_img = random.sample(entire_images, k=int(len(entire_images) * test_ratio))
    ...
    for full_img_name, sub_imgs in sub_img_dict.items():
        if full_img_name in entire_test_img:
            cutting_test_files.extend(sub_imgs)
        else:
            cutting_train_files.extend(sub_imgs)

    return cutting_train_files, entire_test_img  # 返回训练集子图和测试集完整图片

def load_dataset(entire_image_dir, entire_mask_dir, cutting_image_dir, cutting_mask_dir, transform):
    cutting_train_files, entire_test_img = split_dataset(entire_image_dir, cutting_image_dir)
    train_dataset = SegmentationDataset(cutting_image_dir, cutting_mask_dir, cutting_train_files, transform)
    test_dataset = SegmentationDataset(entire_image_dir, entire_mask_dir, entire_test_img, transform)
    
    # 返回训练和测试的 DataLoader 以及测试集的完整数据集
    return DataLoader(train_dataset, batch_size=1, shuffle=True, num_workers=2), DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=2), test_dataset
...
transform = Transform()
# Load datasets
train_loader_cutting, test_loader_entire, test_dataset_entire = load_dataset('data/entire_img/images', 'data/entire_img/masks','data/cutting/images', 'data/cutting/masks', transform)
...
if __name__ == "__main__":
    ...
    for i in range(num_epochs):
        train_loader, test_loader = train_loader_cutting, test_loader_entire
