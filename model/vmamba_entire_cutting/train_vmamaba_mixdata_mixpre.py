...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = Image.open(mask_path).convert("RGB")  #mask是RGB图像
        ...
        # print(f"Image {idx} - Original Mask unique values: {np.unique(mask_np, axis=0)}")

        mask = self.color_to_class(mask_np)
    ...
    def color_to_class(self, mask):
        ...
        # print("mask after conversion:", mask)
        ...
        for color, class_index in COLOR_MAP.items():
            ...
            # 调试信息，检查每个颜色的匹配情况
            # if np.any(matches):
            #     print(f"Matched color {color} to class index {class_index} in mask")
...
def visualize_predictions(model, dataset, indices=[0, 1, 2], num_samples=3, device='cpu'):
    print("开始预测可视化")
    model.eval()
    fig, axes = plt.subplots(num_samples, 4, figsize=(20, 10))
    ...
    print("结束预测可视化")
    plt.tight_layout()
    plt.savefig('model/visual/predictions_entire_or_cutting_images_epoch_{}.png'.format(epoch))
    plt.close()
...
if __name__ == "__main__":
    ...
    all_images = os.listdir('data/mix/images')  # Make sure to use your actual directory path     
    train_files, test_files = train_test_split(all_images, test_size=0.2, random_state=42)

    # Dataset instances for training and testing
    train_dataset = SegmentationDataset('data/mix/images', 'data/mix/masks', train_files, transform=transform)
    test_dataset = SegmentationDataset('data/mix/images', 'data/mix/masks', test_files, transform=transform)
    ...
    # 可视化一些样本以确认数据增强的效果
    visualize_augmentations(train_dataset, indices=[0, 1, 2], num_samples=3)
    ...
    for i in range(epoch):
        ...
        visualize_predictions(model, test_dataset, indices=[0, 1, 2], num_samples=3, device=device)
