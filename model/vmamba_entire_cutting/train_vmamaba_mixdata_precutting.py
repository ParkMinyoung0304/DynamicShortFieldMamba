...
def visualize_predictions_cutting(model, dataset, indices=[0, 1, 2], num_samples=3, device='cpu'):
    print("开始预测可视化")
    model.eval()
    fig, axes = plt.subplots(num_samples, 4, figsize=(20, 10))
    ...
    print("结束预测可视化")
    plt.tight_layout()
    plt.savefig('model/visual/predictions_cutting_epoch_{}.png'.format(num_epochs))
    plt.close()
...
if __name__ == "__main__":
    ...
    for i in range(num_epochs):
        ...
        visualize_augmentations(train_loader_cutting.dataset, indices=[0, 1, 2], num_samples=3)
        ...
        total_iou = 0
        with torch.no_grad():
            for images, labels in test_loader_cutting:
        ...
        avg_test_loss = total_test_loss / len(test_loader_cutting)
        avg_accuracy = total_accuracy / len(test_loader_cutting)
        avg_iou = total_iou / len(test_loader_cutting)
        ...
        visualize_predictions_cutting(model, test_loader_cutting.dataset, indices=[0, 1, 2], num_samples=3, device=device)
