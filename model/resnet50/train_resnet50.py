...
from resnet50 import resnet50  # Assuming VMambaUnet is suitable for segmentation
...
def visualize_predictions_entire_images(model, dataset, indices=[0, 1, 2], num_samples=3, device='cpu'):
    ...
    print("结束预测可视化")
    plt.tight_layout()
    plt.savefig('model/visual/resnet_predictions_entire_images_epoch_{}.png'.format(num_epochs))
    plt.close()
...
def visualize_predictions_all(model, dataset, indices=[0, 1, 2], num_samples=3, device='cpu'):
    ...
    print("结束预测可视化")
    plt.tight_layout()
    plt.savefig('model/visual/resnet_predictions_all_epoch_{}.png'.format(num_epochs))
    plt.close()
